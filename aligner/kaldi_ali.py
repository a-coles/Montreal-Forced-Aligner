...
def ali_to_textgrid_kaldi_func(ali_directory, model_directory, lang_dir, split_data_dir, sym2int_script, oov, job_name):  # pragma: no cover
    ...
    # Now we have word_ctm's with word-tier timestamps/annotations,
    # and phone_ctm's with phone-tier timestamps/annotations.
    # Now build the textgrids.
...
def convert_ali_to_textgrids_kaldi(ali_directory, model_directory, lang_dir, split_data_dir, sym2int_script, oov, num_jobs):
    ...
    corpus_path = '/Users/mlml/Documents/Project/train_si84'
    textgrid_path = '/Users/mlml/Documents/Project/train_si84_ali_tri4b'
    word_ctms = []
    phone_ctms = []
    for file in os.listdir('/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/exp/nnet2_online/tri4b_ali_si84/'):
        if 'word_ctm' in file:
            word_ctms.append(os.path.join('/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/exp/nnet2_online/tri4b_ali_si84/', file))
        if 'phone_ctm' in file:
            phone_ctms.append(os.path.join('/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/exp/nnet2_online/tri4b_ali_si84/', file))
    #word_ctms = glob.glob('/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/exp/nnet2_online/nnet_ms_a/word_ctm.*')
    #phone_ctms = glob.glob('/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/exp/nnet2_online/nnet_ms_a/phone_ctm.*')
    for file in os.listdir(corpus_path):
        if file.endswith('.wav'):
            print("Looking at this wav: {}".format(file))
            file_stem = file.split('.')[0]
            #print("file stem: {}".format(file_stem))
            ...
            # Make textgrid with word and phone tier
            tg = TextGrid(maxTime=maxtime)
            wordtier = IntervalTier(name='words', maxTime=maxtime)
            phonetier = IntervalTier(name='phones', maxTime=maxtime)

            # Fill word tier
            word_index = '/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/data/lang/words.txt'
            for word_ctm in word_ctms:
                with open(word_ctm, 'r') as fp:
                    lines = fp.readlines()
                    for line in lines:
                        line = line.split(' ')
                        fileid = line[0]
                        #print("fileid: {}".format(fileid))
                        beg = float(line[2]) + .00000000000001
                        end = beg + float(line[3]) - .00000000000001
                        word_int = line[4]
                        if fileid in file_stem:
                            with open(word_index, 'r') as wi:
                                wordlines = wi.readlines()
                                for wordline in wordlines:
                                    wordline = wordline.split(' ')
                                    if wordline[1] == word_int:
                                        word = wordline[0]
                                        break
                            wordtier.add(beg, end, word)
            print("Built word tier.")

            # Fill phone tier
            phone_index = '/Users/mlml/Documents/Project/kaldi2/egs/wsj/s5/data/lang/phones.txt'
            for phone_ctm in phone_ctms:
                with open(phone_ctm, 'r') as fp:
                    lines = fp.readlines()
                    for line in lines:
                        line = line.split(' ')
                        fileid = line[0]
                        ...
                        phone_int = line[4]
                        if fileid in file_stem:
                            with open(phone_index, 'r') as pi:
                                phonelines = pi.readlines()
                                for phoneline in phonelines:
                                    phoneline = phoneline.split(' ')
                                    if phoneline[1] == phone_int:
                                        phone = phoneline[0]
                                        break
                            phonetier.add(beg, end, phone)
            print("Built phone tier.")
            ...
            # Write textgrid
            outpath = os.path.join(textgrid_path, file_stem + '.TextGrid')
            tg.write(outpath)
...
if __name__ == '__main__':
    ...
    nnet_dir = os.path.join(kaldi_path, 'egs', 'wsj', 's5', 'exp', 'nnet2_online', 'tri4b_ali_si84')
    data_dir = os.path.join(kaldi_path, 'egs', 'wsj', 's5', 'data', 'train_si84_hires')
    #ali_dir = os.path.join(kaldi_path, 'egs', 'wsj', 's5', 'exp', 'nnet2_online', 'nnet_ms_a', 'ali_final')
    ali_dir = os.path.join(kaldi_path, 'egs', 'wsj', 's5', 'exp', 'nnet2_online', 'tri4b_ali_si84')
    """for (root, dirs, files) in os.walk(data_dir):
        ...
        break"""
    num_jobs = 40
    split_data_dir = os.path.join(data_dir, 'split40')
